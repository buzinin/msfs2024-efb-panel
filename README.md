## MSFS In-Game Panel for EFB

This add-on adds an **in-game panel** with the default tablet (EFB). The goal of this project is to provide a temporary solution while the MSFS VR mode bug with the EFB is not fixed.

### 📌 Installation

1. **Download and extract** the archive (available in the [Releases](#)) into the `Community` folder.
    - **Steam version:** `C:\Users[Your Username]\AppData\Roaming\Microsoft Flight Simulator 2024\Packages\Community`
    - **MS Store version:** `C:\Users[Your Username]\AppData\Local\Packages\Microsoft.Limitless_8wekyb3d8bbwe\LocalCache\Packages\Community`
2. **Start MSFS** and go to the **control settings** to assign a key for opening the toolbar.
3. **Launch a flight.**
4. To open the panel, **click the "EFB PANEL" icon** in the toolbar or use the default EFB hotkey.

### ⚠️ Limitations & Features

- The panel creates a **separate instance** of the EFB. It mostly syncs with the default tablet.
- In aircraft with a **3D model of the EFB**, switching between the 3D tablet and the panel will open the last used app but won't keep subpage navigation.

### 🔧 Troubleshooting

- If you experience issues during gameplay, you can **disable the panel** from the toolbar:
    1. Click the **gear icon** in the toolbar.
    2. Toggle off **EFB Panel**.
- If issues persist, **remove the add-on** from the `Community` folder and restart MSFS.
- Found a bug? **Report it** in the [Issues](#) section of this repository.  
